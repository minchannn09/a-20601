...
# --------------------------------------------------
# [그래프 6] 요일(X축) x 연월(Y축) 관객수 캘린더 히트맵
# --------------------------------------------------
st.header("🗓️ 요일 및 월별 관객 분포 (캘린더 히트맵)")

# 1. 날짜 데이터에서 요일명, 연월, 날짜 문자열(YYYY-MM-DD) 추출
...
# 3. pivot_table을 활용해 히트맵 행(연월), 열(요일) 형태 데이터 구조로 전치(Transpose)
# index를 '연월', columns를 '요일명'으로 변경해 X축=요일, Y축=연월 구성
z_matrix = heatmap_df.pivot_table(
    index="연월", columns="요일명", values="해당일관객수", aggfunc="sum"
).reindex(columns=days_order)

date_matrix = heatmap_df.pivot_table(
    index="연월", columns="요일명", values="날짜문자열", aggfunc="first"
).reindex(columns=days_order)
...
fig6 = go.Figure(
    data=go.Heatmap(
        z=z_matrix.values,
        x=days_kr,  # X축 레이블: 한글 요일 (월요일~일요일)
        y=z_matrix.index,  # Y축 레이블: 연-월 (YYYY-MM)
        customdata=date_matrix.values,  # 마우스 호버용 날짜 문자열
        hovertemplate="<b>날짜: %{customdata}</b><br>요일: %{x}<br>관객수: %{z:,.0f}명<extra></extra>",
